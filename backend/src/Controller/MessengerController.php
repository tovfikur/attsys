...
class MessengerController
{
    ...
    public function conversations()
    {
        ...
        if ($role === 'tenant_owner') {
            $stmt = $pdo->prepare("SELECT c.id, c.tenant_id, c.kind, c.conversation_key, c.direct_employee_a, c.direct_employee_b, c.direct_user_id, c.direct_employee_id, c.created_at, c.updated_at, ea.name AS a_name, ea.code AS a_code, ea.profile_photo_path AS a_photo, eb.name AS b_name, eb.code AS b_code, eb.profile_photo_path AS b_photo, ou.name AS owner_name, oe.name AS owner_employee_name, oe.code AS owner_employee_code, oe.profile_photo_path AS owner_employee_photo FROM messenger_conversations c LEFT JOIN employees ea ON ea.tenant_id=c.tenant_id AND ea.id=c.direct_employee_a LEFT JOIN employees eb ON eb.tenant_id=c.tenant_id AND eb.id=c.direct_employee_b LEFT JOIN tenant_users ou ON ou.tenant_id=c.tenant_id AND ou.id=c.direct_user_id LEFT JOIN employees oe ON oe.tenant_id=c.tenant_id AND oe.id=c.direct_employee_id WHERE c.tenant_id=? ORDER BY (c.id=? ) DESC, c.updated_at DESC, c.id DESC");
            $stmt->execute([(int)$tenantId, (int)$annId]);
        } else {
            $stmt = $pdo->prepare("SELECT c.id, c.tenant_id, c.kind, c.conversation_key, c.direct_employee_a, c.direct_employee_b, c.direct_user_id, c.direct_employee_id, c.created_at, c.updated_at, ea.name AS a_name, ea.code AS a_code, ea.profile_photo_path AS a_photo, eb.name AS b_name, eb.code AS b_code, eb.profile_photo_path AS b_photo, ou.name AS owner_name, oe.name AS owner_employee_name, oe.code AS owner_employee_code, oe.profile_photo_path AS owner_employee_photo FROM messenger_conversations c LEFT JOIN employees ea ON ea.tenant_id=c.tenant_id AND ea.id=c.direct_employee_a LEFT JOIN employees eb ON eb.tenant_id=c.tenant_id AND eb.id=c.direct_employee_b LEFT JOIN tenant_users ou ON ou.tenant_id=c.tenant_id AND ou.id=c.direct_user_id LEFT JOIN employees oe ON oe.tenant_id=c.tenant_id AND oe.id=c.direct_employee_id WHERE c.tenant_id=? AND (c.id=? OR c.direct_employee_a=? OR c.direct_employee_b=? OR c.direct_employee_id=?) ORDER BY (c.id=? ) DESC, c.updated_at DESC, c.id DESC");
